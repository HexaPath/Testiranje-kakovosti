...
public class Bst<Tip extends Comparable> implements Seznam<Tip> {
    ...
    public List<Tip> asList(){
        listaBst.clear();
        rekurzijaListe(rootNode);
        return listaBst;
    }
    
    private void rekurzijaListe(ElementBST node){
        if (node == null){ 
            return;
        }
        rekurzijaListe(node.left);
        listaBst.add(node.value);
        rekurzijaListe(node.right);
    }
}
